   ...
   if (!empty($price_rows) && is_array($price_rows)) {
     foreach ($price_rows as $row) {
       ...
       if ($acf_job_length === $job_length) {
         $book_price       = $acf_hourly_rate * $job_length * $total_workers;
...
<div class="mp-form-row mepr_bold mepr_price">
  <div class="mepr_price_cell invoice-amount">
    <?php
    function get_amount_due_later($entry_id, $product_id = 1728) {
      if (!$entry_id || !class_exists('GFAPI')) return null;
    ...
    $entry_id = isset($_GET['entry']) ? intval($_GET['entry']) : 0;
    $pricing = get_amount_due_later($entry_id);

    if ($pricing && $pricing['amount_due_later'] > 0) {
      echo '<div style="padding-top:21px;padding-bottom:21px;">
              <h4 style="margin:0;font-size:16px;line-height:21px;">Amount due later: $' . number_format($pricing['amount_due_later'], 2) . '</h4>
              <p style="margin:0;font-size:16px;line-height:21px;">Payment must be made in cash directly to the worker(s) upon job completion or at the end of the allotted time frame, whichever comes first.</p>
            </div>';
    }
    ?>
  </div>
